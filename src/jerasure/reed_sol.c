...
int reed_sol_r6_encode(int k, int w, char **data_ptrs, char **coding_ptrs,
		       int size)
{
	int i;
	...
	for (i = 1; i < k; i++)
		galois_region_xor(coding_ptrs[0], data_ptrs[i], coding_ptrs[0],
				  size);
	...
	for (i = k - 2; i >= 0; i--) {
		switch (w) {
		case 8:
			reed_sol_galois_w08_region_multby_2(coding_ptrs[1],
							    size);
			break;
		case 16:
			reed_sol_galois_w16_region_multby_2(coding_ptrs[1],
							    size);
			break;
		case 32:
			reed_sol_galois_w32_region_multby_2(coding_ptrs[1],
							    size);
			break;
		default:
			return 0;
		...
		galois_region_xor(coding_ptrs[1], data_ptrs[i], coding_ptrs[1],
				  size);
	}
	return 1;
}
...
int *reed_sol_big_vandermonde_distribution_matrix(int rows, int cols, int w)
{
	...
	for (i = 1; i < cols; i++) {
		...
		if (dist[sindex + i] != 1) {
			...
			for (j = 0; j < rows; j++) {
				dist[srindex] =
				    galois_single_multiply(tmp, dist[srindex],
							   w);
				srindex += cols;
			}
		}
	...
	for (i = cols + 1; i < rows; i++) {
		...
		if (tmp != 1) {
			...
			for (j = 0; j < cols; j++)
				dist[sindex + j] =
				    galois_single_multiply(dist[sindex + j],
							   tmp, w);
		}
		sindex += cols;
	}
