...
int jerasure_make_decoding_bitmatrix(int k, int m, int w, int *matrix,
				     int *erased, int *decoding_matrix,
				     int *dm_ids)
{
	int i, j, *tmpmat;
	int index, mindex;
			   ...
			   int size)
{
	...
	if (edd > 1 || (edd > 0 && (!row_k_ones || erased[k]))) {
		...
		if (jerasure_make_decoding_matrix
		    (k, m, w, matrix, erased, decoding_matrix, dm_ids) < 0) {
			free(erased);
			free(dm_ids);
	...
	if (edd > 0) {
		...
		jerasure_matrix_dotprod(k, w, matrix, tmpids, lastdrive,
	...
	for (i = 0; i < m; i++) {
		if (erased[k + i]) {
			jerasure_matrix_dotprod(k, w, matrix + (i * k), NULL,
						i + k, data_ptrs, coding_ptrs,
						size);
		}
	}
...
int *jerasure_matrix_to_bitmatrix(int k, int m, int w, int *matrix)
{
	...
	for (i = 0; i < m; i++) {
		...
		for (j = 0; j < k; j++) {
			...
			for (x = 0; x < w; x++) {
				for (l = 0; l < w; l++) {
					bitmatrix[colindex + x + l * rowelts] =
					    ((elt & (1 << l)) ? 1 : 0);
				}
			    ...
			    char **data_ptrs, char **coding_ptrs, int size)
{
	...
	if (w != 8 && w != 16 && w != 32) {
		fprintf(stderr,
			"ERROR: jerasure_matrix_encode() and w is not 8, 16 or 32\n");
		exit(1);
	}
				...
				for (y = 0; y < w; y++) {
					if (bitmatrix_row[index]) {
						...
						if (!pstarted) {
							memcpy(pptr, dptr,
							       packetsize);
							jerasure_total_memcpy_bytes
							    += packetsize;
							pstarted = 1;
						} else {
							galois_region_xor(pptr,
									  dptr,
									  pptr,
									  packetsize);
...
int jerasure_invert_matrix(int *mat, int *inv, int rows, int w)
{
	...
	for (i = 0; i < cols; i++) {
		...
		if (mat[row_start + i] == 0) {
			for (j = i + 1; j < rows && mat[cols * j + i] == 0;
			     j++) ;
			if (j == rows)
				return -1;
			rs2 = j * cols;
		...
		if (tmp != 1) {
			...
			for (j = 0; j < cols; j++) {
				mat[row_start + j] =
				    galois_single_multiply(mat[row_start + j],
							   inverse, w);
				inv[row_start + j] =
				    galois_single_multiply(inv[row_start + j],
							   inverse, w);
			}
		}
...
void jerasure_free_schedule_cache(int k, int m, int ***cache)
{
	...
	if (m != 2) {
		fprintf(stderr,
			"jerasure_free_schedule_cache(): m must equal 2\n");
		exit(1);
	}
			     ...
			     char **data_ptrs, char **coding_ptrs, int size)
{
	...
	for (i = 0; i < k; i++) {
		if (matrix_row[i] != 0 && matrix_row[i] != 1) {
			...
			switch (w) {
			case 8:
				galois_w08_region_multiply(sptr, matrix_row[i],
							   size, dptr, init);
				break;
			case 16:
				galois_w16_region_multiply(sptr, matrix_row[i],
							   size, dptr, init);
				break;
			case 32:
				galois_w32_region_multiply(sptr, matrix_row[i],
							   size, dptr, init);
				break;
			}
...
int jerasure_bitmatrix_decode(int k, int m, int w, int *bitmatrix,
			      int row_k_ones, int *erasures, char **data_ptrs,
			      char **coding_ptrs, int size, int packetsize)
{
	int i, j;
	int *erased;
	int *decoding_matrix;
	...
	if (edd > 1 || (edd > 0 && (row_k_ones != 1 || erased[k]))) {
		...
		if (jerasure_make_decoding_bitmatrix
		    (k, m, w, bitmatrix, erased, decoding_matrix, dm_ids) < 0) {
			free(erased);
			free(dm_ids);
			free(decoding_matrix);
			return -1;
	...
	for (i = 0; edd > 0 && i < lastdrive; i++) {
		if (erased[i]) {
			jerasure_bitmatrix_dotprod(k, w,
						   decoding_matrix +
						   i * k * w * w, dm_ids, i,
						   data_ptrs, coding_ptrs, size,
						   packetsize);
	...
	if (edd > 0) {
		...
		jerasure_bitmatrix_dotprod(k, w, bitmatrix, tmpids, lastdrive,
					   data_ptrs, coding_ptrs, size,
					   packetsize);
		free(tmpids);
	}
						 ...
						 int smart)
{
	...
	if (set_up_ids_for_scheduled_decoding
	    (k, m, erasures, row_ids, ind_to_row) < 0)
		return NULL;
...
/*    printf("\nMatrix to invert\n");
    ...
    printf("\n"); */
		...
		for (i = 0; i < ddf; i++) {
			memcpy(ptr, inverse + k * w * w * row_ids[k + i],
			       sizeof(int) * k * w * w);
			ptr += (k * w * w);
		}
		free(inverse);
	...
	for (x = 0; x < cdf; x++) {
		...
		memcpy(ptr, bitmatrix + drive * k * w * w,
		       sizeof(int) * k * w * w);
		...
		for (i = 0; i < k; i++) {
			if (row_ids[i] != i) {
				b1 = real_decoding_matrix + (ind_to_row[i] -
							     k) * k * w * w;
				for (j = 0; j < w; j++) {
					b2 = ptr + j * k * w;
					for (y = 0; y < w; y++) {
...
/*
  ...
  printf("\n"); */
	if (smart) {
		schedule =
		    jerasure_smart_bitmatrix_to_schedule(k, ddf + cdf, w,
							 real_decoding_matrix);
	} else {
		schedule =
		    jerasure_dumb_bitmatrix_to_schedule(k, ddf + cdf, w,
							real_decoding_matrix);
	}
	free(row_ids);
	free(ind_to_row);
				  ...
				  int smart)
{
	...
	ptrs =
	    set_up_ptrs_for_scheduled_decoding(k, m, erasures, data_ptrs,
					       coding_ptrs);
	if (ptrs == NULL)
		return -1;

	schedule =
	    jerasure_generate_decoding_schedule(k, m, w, bitmatrix, erasures,
						smart);
	if (schedule == NULL) {
		free(ptrs);
		return -1;
					...
					int smart)
{
	...
	for (e1 = 0; e1 < k + m; e1++) {
		...
		for (e2 = 0; e2 < e1; e2++) {
			...
			erasures[2] = -1;
			scache[e1 * (k + m) + e2] =
			    jerasure_generate_decoding_schedule(k, m, w,
								bitmatrix,
								erasures,
								smart);
		...
		erasures[1] = -1;
		scache[e1 * (k + m) + e1] =
		    jerasure_generate_decoding_schedule(k, m, w, bitmatrix,
							erasures, smart);
	}
	return scache;
...
int jerasure_invert_bitmatrix(int *mat, int *inv, int rows)
{
	...
	for (i = 0; i < cols; i++) {
		...
		if ((mat[i * cols + i]) == 0) {
			for (j = i + 1; j < rows && (mat[j * cols + i]) == 0;
			     j++) ;
			if (j == rows)
				return -1;
			for (k = 0; k < cols; k++) {
			      ...
			      int w)
{
	...
	for (i = 0; i < r1; i++) {
		for (j = 0; j < c2; j++) {
			for (k = 0; k < r2; k++) {
				product[i * c2 + j] ^=
				    galois_single_multiply(m1[i * c1 + k],
							   m2[k * c2 + j], w);
			}
		}
	}
			      ...
			      int packetsize)
{
	...
	for (tdone = 0; tdone < size; tdone += packetsize * w) {
		jerasure_do_scheduled_operations(ptr_copy, schedule,
						 packetsize);
		for (i = 0; i < k + m; i++)
			       ...
			       int packetsize)
{
	...
	if (packetsize % sizeof(long) != 0) {
		fprintf(stderr,
			"jerasure_bitmatrix_encode - packetsize(%d) \% sizeof(long) != 0\n",
			packetsize);
		exit(1);
	}
	...
	for (i = 0; i < m; i++) {
		jerasure_bitmatrix_dotprod(k, w, bitmatrix + i * k * w * w,
					   NULL, k + i, data_ptrs, coding_ptrs,
					   size, packetsize);
	}
}
