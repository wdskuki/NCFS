...
private:
	...
	typedef struct{
		...
		char name[32];             // device/file name (assume 32 byte max)
		int id;
	} Cache_Entry;
...
public:
	// public functions
	CacheLayer(struct ecfs_state* ecfs_data);
	...
	int FileInfoWrite(int id, const char* buf,long long size, long long offset);
